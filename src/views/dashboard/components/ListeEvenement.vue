<template>
  <div class="bg-white shadow-md rounded-lg p-4">
    ...
    <div v-if="activeTab === 'list'" class="bg-white shadow-md rounded-lg p-4">
      <h3 class="text-lg font-semibold text-[#0297B8] mb-4">Liste des Événements</h3>
      <table class="min-w-full border border-gray-200 rounded-lg">
        ...
        <tbody>
          <tr v-for="(event, index) in paginatedEvents" :key="index" class="border-b hover:bg-gray-100">
            ...
            <td class="p-3">
              <button class="text-[#0297B8] font-semibold hover:underline"
                      @click="voirParticipants(event)">
                {{ event.participation }}
              </button>
            </td>
      ...
      <div class="flex justify-between items-center mt-4">
        ...
        <div class="flex gap-2">
          ...
          <button v-for="page in totalPages" :key="page" @click="currentPage = page"
                  :class="['px-3 py-1 border rounded hover:bg-gray-200', currentPage === page ? 'bg-[#0297B8] text-white' : '']">
            {{ page }}
          </button>
