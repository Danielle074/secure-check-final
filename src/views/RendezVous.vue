<template>
  <div class="min-h-screen bg-gray-100 flex items-center justify-center p-6">
    <div class=" w-full max-w-4xl p-8">
      <h2 class="text-3xl font-bold text-center text-[#02739A] mb-8">
      ...
      <form @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select v-model="form.idType" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-[#02739A] bg-gray-200">
        ...
        <div>
          ...
          <input v-model="form.idNumber" type="text" placeholder="Ex : C1234567"
            class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-[#02739A] bg-gray-200" />
        </div>
        ...
        <div>
          ...
          <div class="relative">
            <select v-model="form.country" class="w-full border rounded-lg p-2 appearance-none focus:ring-2 focus:ring-[#02739A] bg-gray-200">
        ...
        <div class="md:col-span-2">
          ...
          <textarea v-model="form.reason" rows="3" placeholder="Décrivez brièvement la raison"
            class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-[#02739A] bg-gray-200"></textarea>
        </div>

        <!-- Date -->
        <div>
          <label class="block text-gray-700 font-medium mb-2">Sélectionner une date</label>
          <input v-model="form.date" type="date" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-[#02739A] bg-gray-200" />
        </div>

        <!-- Heure -->
        <div>
          <label class="block text-gray-700 font-medium mb-2">Sélectionner une heure</label>
          <input v-model="form.time" type="time" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-[#02739A] bg-gray-200" />
        </div>
        ...
        <div class="md:col-span-2 flex justify-center mt-4">
          <button type="submit"
            class="bg-[#02739A] text-white px-8 py-3 rounded-lg font-semibold hover:bg-[#025d7a] transition-all ">
            Envoyer la demande
          </button>
        </div>
      </form>
